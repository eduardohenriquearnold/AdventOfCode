...
while i < tilesInRow:
    while j < tilesInRow:
        if j == 0:
            ...
            # adjust flipping
            if (
                getHash(tiles[ids[i - 1, j]], False)[1]
                != getHash(tiles[ids[i, j]], False)[3]
            ):
            ...
            # adjust flipping
            if (
                getHash(tiles[ids[i, j - 1]], False)[2]
                != getHash(tiles[ids[i, j]], False)[0]
            ):
                tiles[ids[i, j]] = tiles[ids[i, j]][::-1, :]
        j += 1
    j = 0
...
img = np.concatenate(
    [
        np.concatenate(
            [tiles[ids[i, j]][1:-1, 1:-1] for j in range(tilesInRow)], axis=1
        )
