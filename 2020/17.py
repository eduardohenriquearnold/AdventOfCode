...
def activeNeigh(c, *argv) -> int:
    ...
    assert len(c.shape) == len(
        argv
    ), "number of coordinates must match the number of dimensions of cube"
...
c = [
    list(map(int, l.rstrip().replace("#", "1").replace(".", "0")))
    for l in open("./inputs/17", "r").readlines()
]
